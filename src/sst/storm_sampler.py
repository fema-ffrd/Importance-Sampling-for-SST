...
def sample_storm_centers(sp_domain: gpd.GeoDataFrame, dist_x=None, dist_y=None, dist_xy=None, num_simulations=10000) -> pd.DataFrame:
    '''Get storm center samples.
    ...
    Args:
        ...
        dist_x (scipy.stats.*, optional): Univariate Scipy distribution object for x samples. Use None for uniform sampling. Defaults to None.
        dist_y (scipy.stats.*, optional): Univariate Scipy distribution object for y samples. Use None for uniform sampling. Defaults to None.
        dist_xy (scipy.stats.*, optional): Bivariate Scipy distribution object for x and y samples. Use None for uniform Sampling. Defaults to None.
        num_simulations (int, optional): Number of simulations. Defaults to 10000.
    ...
    Notes:
        - Set dist_x, dist_y, and dist_xy to None to perform uniform sampling.
        - Use dist_x and dist_y and set dist_xy to None to perform independent univariate sampling.
        - Use dist_xy and set dist_x and dist_y to None to perform bivariate sampling.
    '''
    v_domain_stats = get_sp_stats(sp_domain)

    if dist_x is None and dist_xy is None:
        method = 0
    ...
    while len(v_centroid_x) < num_simulations and i <= max_iter:
        ...
        if method == 2:
            _v_centroid_xy = dist_xy.rvs(num_simulations)
            _v_centroid_x = [float(_[0]) for _ in _v_centroid_xy]
            _v_centroid_y = [float(_[1]) for _ in _v_centroid_xy]
        else: 
    ...
    if method == 1:
        ...
        _v_centroid_xy = [[_x, _y] for _x, _y in zip(v_centroid_x, v_centroid_y)]

        p = 1
        q = dist_xy.pdf(_v_centroid_xy)
        v_weight = p / q
...
def sample_storms(df_storms: pd.DataFrame, sp_domain: gpd.GeoDataFrame, dist_x=None, dist_y=None, dist_xy=None, num_simulations=10000):
    ...
    _df_storm_centers = sample_storm_centers(sp_domain=sp_domain, dist_x=dist_x, dist_y=dist_y, dist_xy=dist_xy, num_simulations=num_simulations)
