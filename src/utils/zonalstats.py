    ...
    try:
        ...
        with rio.open(raster_path) as src:
            ...
            if src.dtypes[0] in ['float32', 'float64'] or raster_nodata is None and np.isnan(masked_data).any():
                ...
                valid_values = masked_data[~np.isnan(masked_data)] if masked_data.dtype.kind == 'f' else masked_data[masked_data != raster_nodata]
...
def _sum_raster_values_in_polygon_rasterstats(raster_path: str, gdf: gpd.geodataframe) -> float:
    try:
        ...
        stats_results = rasterstats.zonal_stats(
            ...
            raster=raster_path,
            stats=["mean"],         # We only need the sum
        ...
        return float(stats_results[0]['mean']) if stats_results and stats_results[0]['mean'] is not None else np.nan
...
def _sum_raster_values_in_polygon_xarray(raster_path: str, gdf: gpd.geodataframe) -> float:
    try:
        ...
        valid_data = clipped_da.data[~np.isnan(clipped_da.data)]
...
def raster_zonal_mean(raster_path: str, gdf: gpd.geodataframe, backend: Literal['rasterio', 'rasterstats' 'xarray']='xarray') -> float:
    match backend:
        case 'rasterio':
